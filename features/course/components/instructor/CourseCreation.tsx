...
export default function CourseCreation({
  ...
  const { createCourse, editCourseCreationDetailsApi } = useCourseApi();
  ...
  const handleCreateCourse = async (formData: CourseFormData) => {
    try {
      ...
      if (thumbnailSignedUrl.status == "error") {
        console.log("Error");
        return;
      }
      ...
      if (videoSignedUrl.status == "error") {
        console.log("Error");
        return;
      }
      // Uploading files to s3
      await axios.put(thumbnailSignedUrl.data.url, imageFile, {
        headers: { "Content-Type": thumbnailContentType },
      });
      await axios.put(videoSignedUrl.data.url, videoFile, {
      ...
      router.push(`/instructor/courses/create/${response.data.id}`); // Navigate to courses page after success
    } catch (error) {
      let commonError;
    ...
    try {
      ...
      if (videoFile) {
        ...
        if (videoSignedUrlResponse.status == "error") {
          console.log("error in video signed url");
          return;
        }
        // Uploading files to s3
      ...
      if (imageFile) {
        ...
        const thumbnailContentType = imageFile.type;
        const thumbnailSignedUrlResponse = await generatePutSignedUrlApi({
          contentType: thumbnailContentType,
          isPublic: false,
          isTemp: true,
          key: thumbnailKey,
        });
        if (thumbnailSignedUrlResponse.status == "error") {
          console.log("error in signed url generation");
          return;
        }

        // Uploading files to s3
        await axios.put(thumbnailSignedUrlResponse.data.url, imageFile, {
          headers: { "Content-Type": thumbnailContentType },
        });
      }
      const sanitizedDescription = DOMPurify.sanitize(formData.description);
      ...
      const response = await editCourseCreationDetailsApi(
