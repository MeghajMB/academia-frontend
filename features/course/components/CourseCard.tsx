...
export default function CourseCard({ course }: { course: ListCourses }) {
  ...
  return (
    <Link href={`/course/${course.id}`} className="block">
      <Card
        className="w-full max-w-[340px] h-[470px] transition-all bg-gray-900/50 border border-purple-500/20 duration-300 hover:scale-[1.02] hover:shadow-lg"
        shadow="sm"
      >
        ...
        <CardHeader className="pb-0 pt-3 px-4 flex-col items-start">
          <h3 className="font-bold text-lg line-clamp-2 h-[56px]">
            {course.title}
          </h3>
        </CardHeader>

        <CardBody className="py-2 px-4 flex flex-col gap-2">
          <p>By <span className="text-purple-500">-{course.instructorDetails.name}</span></p>
          <p className="text-default-500 text-sm line-clamp-2 h-[40px]">
            {course.subtitle}
          </p>
          ...
          <div className="flex items-center gap-1 mt-1">
            ...
            <div className="flex">
              {[...Array(course.rating.toFixed(0))].map((_, i) => (
                <Star
                  key={i}
                  size={16}
                  className={`${
